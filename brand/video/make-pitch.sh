#!/usr/bin/env bash
# Records pitch-visual.mp4: the deployed hook in action on a local fork of Robinhood Chain mainnet.
set -euo pipefail
cd "$(dirname "$0")"
...
RAW=$(SITE=http://localhost:$SITE_PORT FORK=$R node record-pitch.cjs | tail -1)
ffmpeg -y -loglevel error -i "$RAW" -c:v libx264 -preset slow -crf 20 -pix_fmt yuv420p -movflags +faststart -r 30 pitch-visual.mp4
ffprobe -v error -show_entries format=duration -of csv=p=0 pitch-visual.mp4
# Narration (subtitles are drawn in-page during recording).
ffmpeg -y -loglevel error -i pitch-visual.mp4 -i ../voice/pitch-Achird.wav -map 0:v -map 1:a \
  -c:v copy -c:a aac -b:a 192k -shortest -movflags +faststart pitch.mp4
